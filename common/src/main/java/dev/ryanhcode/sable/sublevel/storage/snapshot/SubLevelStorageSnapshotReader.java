...
public final class SubLevelStorageSnapshotReader {
    ...
    private static void captureRegion(final Path folder, final Path regionPath, final List<StoredSubLevel> result) throws IOException {
        ...
        for (int index = 0; index < spans.length; index++) {
            ...
            final CompoundTag holdingTag = readTag(regionPath, externalRegionFolder, index, SubLevelRegionFile.SECTOR_SIZE);
            if (holdingTag == null) {
    ...
    private static UUID readSubLevelUuid(final Path folder, final GlobalSavedSubLevelPointer pointer) throws IOException {
        ...
        final CompoundTag subLevelTag = readTag(storagePath, externalFolder, pointer.subLevelIndex(), 4096);
        if (subLevelTag == null) {
    ...
    private static CompoundTag readTag(final Path path, final Path externalFolder, final int index, final int sectorSize) throws IOException {
    ...
    private static void readFully(final FileChannel channel, final ByteBuffer buffer, final long offset) throws IOException {
    ...
    private record FileSnapshot(long size, long modifiedMillis, Object fileKey) {
        private static FileSnapshot capture(final Path path) throws IOException {
            ...
            return new FileSnapshot(attributes.size(), attributes.lastModifiedTime().toMillis(), attributes.fileKey());
        }

        private void verifyUnchanged(final Path path) throws IOException {
            final FileSnapshot current = capture(path);
            if (this.size != current.size || this.modifiedMillis != current.modifiedMillis || !Objects.equals(this.fileKey, current.fileKey)) {
