...
public final class ServerSubLevelLocationSnapshotSelfTest {
    ...
    private static void differentStoredPointerConflicts() {
        ...
        final ServerSubLevelLocationSnapshot.Accumulator accumulator =
                new ServerSubLevelLocationSnapshot.Accumulator();
        accumulator.register(uuid, new ServerSubLevelLocationSnapshot.Loaded(Level.OVERWORLD, 3, 4, recoveryPointer));
        accumulator.register(uuid, new ServerSubLevelLocationSnapshot.Stored(Level.OVERWORLD, pointer(0, 0, 0, 2)));
        final ServerSubLevelLocationSnapshot.Snapshot snapshot = accumulator.finish();
    ...
    private static void crossDimensionDuplicateConflicts() {
        ...
        final ServerSubLevelLocationSnapshot.Accumulator accumulator =
                new ServerSubLevelLocationSnapshot.Accumulator();
        accumulator.register(uuid, new ServerSubLevelLocationSnapshot.Loaded(Level.OVERWORLD, 5, 6, pointer));
        accumulator.register(uuid, new ServerSubLevelLocationSnapshot.Stored(Level.NETHER, pointer));
        final ServerSubLevelLocationSnapshot.Snapshot snapshot = accumulator.finish();
        assert snapshot.status() == ServerSubLevelLocationSnapshot.Status.CONFLICTED;
        assert snapshot.conflicts().size() == 1;
    }
    ...
    private static GlobalSavedSubLevelPointer pointer(final int chunkX, final int chunkZ, final int storageIndex, final int subLevelIndex) {
        return new GlobalSavedSubLevelPointer(new ChunkPos(chunkX, chunkZ), (short) storageIndex, (short) subLevelIndex);
    }
}
