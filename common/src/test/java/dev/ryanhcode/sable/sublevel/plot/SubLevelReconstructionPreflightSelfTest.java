...
public final class SubLevelReconstructionPreflightSelfTest {
    ...
    private static void validSnapshotIsAccepted() {
        ...
        assert result.targetSlot().orElseThrow().equals(new SubLevelReconstructionPreflight.TargetSlot(1, 2));
    }
    ...
    private static void invalidUuidDependencyAndBoundsAreRejected() {
        ...
        final SubLevelData invalid = new SubLevelData(parsed.uuid(), parsed.bounds(), parsed.pose(), List.of(parsed.uuid()), tag);
    ...
    private static SubLevelReconstructionPreflight.Result validate(final SubLevelData data) {
        return SubLevelReconstructionPreflight.validateSerialized(data, LOG_SIDE_LENGTH, LOG_PLOT_SIZE, SECTION_COUNT);
    }
